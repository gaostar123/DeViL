...
from . import transform_video as T
...
def load_video(
    ...
    # Calculate timestamps starting from 0
    if fps is not None:
        timestamps = np.arange(0, duration + 1 / fps, 1 / fps)[:len(frames)]
    else:
        timestamps = np.linspace(0, duration, len(frames))
    ...
    imgs = [Image.fromarray(np.transpose(f, (1, 2, 0)).astype('uint8')).convert('RGB') for f in frames]
    frames = [frame for frame in frames]

    return frames, timestamps, imgs
...
def load_video_new(
    ...
    make_pil: bool = False,
    verbose: bool = False,
    exact_num_frames: int = 64,
    hwaccel: str | None = None,
):
    # ---- meta ----
    info = _probe_cached(video_path)
    v = next(s for s in info["streams"] if s["codec_type"] == "video")
    w, h = int(v["width"]), int(v["height"])
    full_dur = float(info["format"].get("duration", 0.0))

    t0 = start_time if start_time is not None else float(v.get("start_time", 0.0) or 0.0)
    t1 = end_time if end_time is not None else (full_dur if full_dur > 0 else t0 + 9999.0)
    duration = max(0.0, t1 - t0)
    if duration <= 1e-6:
        duration = 1.0

    fps_out = max(1e-6, (exact_num_frames - 1) / duration + 1e-6)
    ...
    if size is not None:
        ...
        new_w, new_h = w, h
    new_w = max(size_divisible, (new_w // size_divisible) * size_divisible)
    new_h = max(size_divisible, (new_h // size_divisible) * size_divisible)

    input_kwargs = {'ss': t0, 't': (t1 - t0)}
    if hwaccel:
    ...
    stream = stream.filter("fps", fps=fps_out, round="up")
    
    if new_w != w or new_h != h:
        stream = stream.filter("scale", new_w, new_h, flags='bilinear')
        
    stream = ffmpeg.output(
        stream, "pipe:", format="rawvideo", pix_fmt="rgb24",
        loglevel=("info" if verbose else "error")
    )
    out, _ = ffmpeg.run(stream, capture_stdout=True, capture_stderr=not verbose)

    if len(out) == 0:
        arr = np.zeros((1, new_h, new_w, 3), dtype=np.uint8)
    else:
        arr = np.frombuffer(out, np.uint8).reshape((-1, new_h, new_w, 3))

    N = arr.shape[0]
    target = exact_num_frames

    if N == 0:
        arr = np.zeros((target, new_h, new_w, 3), dtype=np.uint8)
    elif N < target:
        pad = np.repeat(arr[-1:], target - N, axis=0)
        arr = np.concatenate([arr, pad], axis=0)
    elif N > target:
        idx = np.linspace(0, N - 1, target).round().astype(np.int32)
        arr = arr[idx]

    frames_array = np.transpose(arr, (0, 3, 1, 2))

    timestamps = np.linspace(0.0, duration, target, dtype=np.float32).tolist()

    imgs = [Image.fromarray(x).convert("RGB") for x in arr] if make_pil else []
    
    frames = list(frames_array)
    
    assert len(frames) == exact_num_frames and len(timestamps) == exact_num_frames, \
        f"Expected {exact_num_frames}, got {len(frames)} / {len(timestamps)}"
...
#     probe = ffmpeg.probe(video_path)
#     duration = float(probe['format']['duration'])
#     duration_ori = duration
#     video_stream = next((stream for stream in probe['streams'] if stream['codec_type'] == 'video'), None)
#     fps_str = video_stream.get('r_frame_rate', '0/1')
#     numerator, denominator = map(int, fps_str.split('/'))
#     video_fps = float(numerator / denominator) if denominator != 0 else 0.0
#     w, h = int(video_stream['width']), int(video_stream['height'])
#     nb_frame=int(video_stream['nb_frames'])
#     kwargs, input_kwargs, output_kwargs = {}, {}, {}
#     do_trim = start_time is not None or end_time is not None
#     if start_time is not None:
#         new_start_time = max(float(video_stream['start_time']), start_time)
...
#     # Uniformly sample 64 frames by extracting at specific timestamps
#     if duration > 0:
#         sample_times = np.linspace(0, duration, 128)
#     else:
#         sample_times = np.zeros(128)

#     frames_list = []
#     frame_time = []
#     for idx, i in enumerate(sample_times):
#         if i < duration_ori:
#             frame_time.append(i)
#         else:
#             frame_time.append(frame_time[idx-1])

#     cap = cv2.VideoCapture(video_path)
#     for frame_num in frame_time:
...
#     cap.release()
#     frames_list = [cv2.cvtColor(f, cv2.COLOR_BGR2RGB).transpose(2, 0, 1) for f in frames_list]
#     frames =  np.stack(frames_list, axis=0)

#     timestamps = sample_times.tolist()

#     # Compute effective fps for padding if needed
#     if duration > 0:
#         effective_fps = 127 / duration  # Since 64 frames over duration, 63 intervals
#     else:
#         effective_fps = 30  # Default

#     if temporal_factor > 1:
#         pad_length = temporal_factor - len(frames) % temporal_factor
#         if pad_length > 0 and pad_length != temporal_factor:
#             frames = np.concatenate([frames, frames[-1:].repeat(pad_length, axis=0)])
#             for _ in range(pad_length):
#                 timestamps.append(timestamps[-1] + 1 / effective_fps)
...
class FallbackTransform(object):
    def __init__(self, primary, fallback):
        self.primary = primary
        self.fallback = fallback

    def __call__(self, img, target):
        img_out, target_out = self.primary(img, target)
        if all(not v for v in target_out['valid']):
            return self.fallback(img, target)
...
def make_coco_transforms(image_set, max_size=640):
    ...
    if image_set == 'train':
        ...
        return T.Compose([
            T.RandomSelect(
                branch1,
                FallbackTransform(branch2, branch1),
            ),
