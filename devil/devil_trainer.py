...
from transformers.utils import (
    is_sagemaker_mp_enabled, is_torch_xpu_available, is_torch_mlu_available,
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_encoder', 'vision_resampler','g_dino']
...
class DeViLTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.llm_lr is not None:
                lm_parameters = [
                    name for name, _ in optimized_parameters if "vision_encoder" not in name and "mm_projector" not in name and "g_dino" not in name
                ]
            ...
            if self.args.g_dino_lr is not None:
                g_dino_parameters = [name for name, _ in optimized_parameters if "g_dino" in name]
                decay_g_dino_parameters = [name for name in g_dino_parameters if name in decay_parameters]
                nodecay_g_dino_parameters = [name for name in g_dino_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
                        "params": [p for n, p in optimized_parameters if n in decay_g_dino_parameters], 
                        "weight_decay": self.args.weight_decay,
                        "lr": self.args.g_dino_lr,
                    },
                    {
                        "params": [p for n, p in optimized_parameters if n in nodecay_g_dino_parameters],
                        "weight_decay": 0.0,
                        "lr": self.args.g_dino_lr,
                    }
                ])
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'is_alignment', False):
            ...
            # Only save Adapter
            keys_to_match = ['mm_projector', 'vision_resampler', 'g_dino']
            ...
            if self.args.lora_enable:
                ...
                super(DeViLTrainer, self)._save_checkpoint(model, trial)
            else:
                super(DeViLTrainer, self)._save_checkpoint(model, trial)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'is_alignment', False):
            ...
            super(DeViLTrainer, self)._save(output_dir, state_dict)
