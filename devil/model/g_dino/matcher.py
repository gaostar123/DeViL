...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1,
                 cost_mask: float = 1, cost_dice: float = 1, cost_proj: float = 1, cost_temporal: float = 1, num_classes: int = 1):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        for i in range(bs):
            ...
            if nf > 1:
                for t in range(nf - 1):
                    # GIoU between boxes of the same query in adjacent frames
                    out_bbox_t = out_bbox[t] # [nq, 4]
                    out_bbox_t_plus_1 = out_bbox[t+1] # [nq, 4]
                    
                    # The cost is 1 - GIoU. We want high GIoU, so low cost.
                    # generalized_box_iou returns a matrix [nq, nq]. We only need the diagonal.
                    cost_temporal_split = -torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(out_bbox_t),
                                                                          box_cxcywh_to_xyxy(out_bbox_t_plus_1)))
                    cost_temporal.append(cost_temporal_split)
                
                # Average cost over all frame pairs, result shape is [nq]
                cost_temporal = torch.stack(cost_temporal, dim=0).mean(0)
                cost_temporal = cost_temporal.unsqueeze(-1) # Reshape to [nq, 1] for broadcasting
            else:
                # If only one frame, temporal cost is zero
                cost_temporal = torch.zeros_like(cost_giou)

            # Final cost matrix
            C = self.cost_class * cost_class + self.cost_bbox * cost_bbox + self.cost_giou * cost_giou + self.cost_temporal * cost_temporal
...
def build_matcher(args, num_classes=1):
    return HungarianMatcher(cost_class=args.set_cost_class,
                            ...
                            cost_temporal=args.set_cost_temporal,
