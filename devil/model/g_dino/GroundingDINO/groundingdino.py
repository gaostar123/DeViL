...
from ..dino_util import get_tokenlizer
from ..dino_util.misc import (
    NestedTensor,
    inverse_sigmoid,
...
# from .temporal_enhancement import TemporalEnhancement
...
class GroundingDINO(nn.Module):
    ...
    def __init__(
        ...
        self.llm_feat_map = nn.Linear(llm_feat_dim, self.hidden_dim, bias=True)
        nn.init.constant_(self.llm_feat_map.bias.data, 0)
        nn.init.xavier_uniform_(self.llm_feat_map.weight.data)
        # freeze
        ...
        # _temporal_enhancement = TemporalEnhancement(hidden_dim, temporal_layer, nheads, 1.0e-5, dropout)
        # temporal_enhancement_list = [_temporal_enhancement for i in range(transformer.num_decoder_layers)]
        # self.temporal_enhancement = nn.ModuleList(temporal_enhancement_list)
        self.tracking_alpha = tracking_alpha
        
        self.attention_maps = None
        self.feature_spatial_shapes = None
    ...
    def forward(self, samples: NestedTensor, llm_tbox_embedding, targets, **kw):
        ...
        # }
        if llm_tbox_embedding is not None:
            if llm_tbox_embedding.dim() == 1:
                llm_tbox_embedding = llm_tbox_embedding.unsqueeze(0)
                
            projected_embedding = self.llm_feat_map(llm_tbox_embedding.to(device))
            
            encoded_text = projected_embedding.unsqueeze(1)
            
            batch_size = encoded_text.shape[0]
            
            text_token_mask = torch.ones((batch_size, 1), dtype=torch.bool, device=device)
            position_ids = torch.zeros((batch_size, 1), dtype=torch.long, device=device)
            text_self_attention_masks = torch.ones((batch_size, 1, 1), dtype=torch.bool, device=device)
        ...
        if not self.training:
            self.attention_maps = []
            if hasattr(self.transformer.encoder, 'fusion_layers') and self.transformer.encoder.fusion_layers:
                for layer in self.transformer.encoder.fusion_layers:
                    if hasattr(layer.attn, 'attention_map') and layer.attn.attention_map is not None:
                        self.attention_maps.append(layer.attn.attention_map)
            
            if hasattr(self.transformer, 'feature_spatial_shapes') and self.transformer.feature_spatial_shapes is not None:
                self.feature_spatial_shapes = self.transformer.feature_spatial_shapes
        ...
        #         zip(reference[:-1], self.bbox_embed, self.class_embed, self.temporal_enhancement, hs)
        # ):
        ...
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_cls_embed, layer_hs) in enumerate(
            ...
            #     BT, Q, D = layer_hs.shape
            #     device = layer_hs.device
            #     layer_hs_temporal = rearrange(layer_hs, '(b t) q d -> b (t q) d', b=B, t=T)
            ...
            #     frame_pos = repeat(pe, 't d -> (t q) d', q=Q).unsqueeze(0).to(dtype=torch.float16)
                
            #     L = T * Q
            #     token_indices = torch.arange(L, device=device)
            #     frame_indices = token_indices // Q
                
            #     row_frame_indices = frame_indices.unsqueeze(1)
            #     col_frame_indices = frame_indices.unsqueeze(0)
                
            #     past_frames_mask = row_frame_indices > col_frame_indices
            #     self_attn_mask = torch.eye(L, L, device=device, dtype=torch.bool)
                
            #     temporal_attn_mask = past_frames_mask | self_attn_mask
                
            #     temporal_attn_mask = temporal_attn_mask.unsqueeze(0).unsqueeze(0).expand(B, -1, -1, -1)
                
            #     temporal_out_stack = layer_temp(
            #         tokens=layer_hs_temporal,
            #         frame_pos=frame_pos,
            #         mask=temporal_attn_mask
            #     )
            #     final_temporal_out = temporal_out_stack[-1]
            #     layer_hs = rearrange(final_temporal_out, 'b (t q) d -> (b t) q d', b=B, t=T, q=Q)
        ...
        last_hs = hs[-1]
        last_hs = rearrange(last_hs, '(b t) q d -> b t q d', b=B, t=T)
        out["pred_inst_embeds"] = last_hs
        ...
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class_list, outputs_coord_list, outputs_mask_list, hs)
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_mask, hs):
        ...
        # else:
        return [
            {"pred_logits": a, "pred_boxes": b, "pred_inst_embeds": c}
...
def build_groundingdino(args, num_classes):
    ...
    model = GroundingDINO(
        ...
        llm_feat_dim=getattr(args, "llm_feat_dim", None),
    )
