...
"""PyTorch DeViL vision encoder model."""
...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "configuration_devil_encoder",
        osp.join(osp.dirname(__file__), "configuration_devil_encoder.py"),
    )
    configuration_devil_encoder = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(configuration_devil_encoder)
    DeViLVisionEncoderConfig = getattr(
        configuration_devil_encoder,
        "DeViLVisionEncoderConfig",
    )
...
class DeViLVisionEmbeddings(nn.Module):

    def __init__(self, config: DeViLVisionEncoderConfig):
        super().__init__()
        self.config = config
...
class DeViLVisionMLP(nn.Module):
...
class DeViLVisionEncoderLayer(nn.Module):
    ...
    def __init__(self, config: DeViLVisionEncoderConfig):
        ...
        self.mlp = DeViLVisionMLP(config)
...
class DeViLVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, config: DeViLVisionEncoderConfig):
        ...
        self.layers = nn.ModuleList([DeViLVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class DeViLVisionEncoderModel(PreTrainedModel):

    config_class = DeViLVisionEncoderConfig
    base_model_prefix = "devil"
    main_input_name = "pixel_values"
    supports_gradient_checkpointing = True
    _no_split_modules = [
        "DeViLVisionEncoderLayer",
        "DeViLVisionEmbeddings",
    ]
    ...
    def __init__(self, config: DeViLVisionEncoderConfig):
        ...
        self.embeddings = DeViLVisionEmbeddings(config)
        self.encoder = DeViLVisionTransformerEncoder(config)
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, DeViLVisionMLP):
