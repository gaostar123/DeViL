...
from devil.constants import (IGNORE_INDEX, DEFAULT_OUTPUT_BOX_TOKEN, DEFAULT_QUERY_START_TOKEN, DEFAULT_QUERY_END_TOKEN,
    NUM_FRAMES, DEFAULT_IMAGE_TOKEN, STREAM_MAX_FRAMES,
    DEFAULT_G_DINO_CONFIG_PATH,
    STREAM_START_TOKEN, STREAM_END_TOKEN)
from devil.mm_utils import (load_images, load_video, load_video_new,
                                  tokenizer_multimodal_token)
from devil.model import *
from devil.devil_trainer import (
    DeViLTrainer, find_all_linear_names, get_peft_state_maybe_zero_3,
    get_peft_state_non_lora_maybe_zero_3, safe_save_model_for_hf_trainer)
from devil.model.processor import DeViLProcessor
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="devil", metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
    ...
    g_dino_config_path: Optional[str] = field(default=DEFAULT_G_DINO_CONFIG_PATH)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    auto_merge_on_resume: bool = field(
        default=True,
        metadata={"help": "When resuming from an existing checkpoint, first merge its LoRA and non-LoRA weights into a full model and save it separately (for backup or inference only), then continue training from that checkpoint step."}
    )
    merge_output_subdir: Optional[str] = field(
        default=None,
        metadata={"help": "Subdirectory name under the checkpoint directory for saving the merged full model. Defaults to 'llm_lo_merge'."}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def _convert_normal(self, data_dict):
        ...
        if 'image' in data_dict and data_dict['image'] is not None:
            ...
            imgs_gt.append(images[0])
            bbox_dict = data_dict.get("ground_truth", {}).get("bboxs", [None]*len(images))[0]
            bbox = [1.0, bbox_dict['xmin'], bbox_dict['ymin'], bbox_dict['xmax'], bbox_dict['ymax']]
            bboxs.append(bbox)
            ...
            if isinstance(video_file, list) and len(video_file) == 1:
                ...
                if ("used_segment" in data_dict and data_dict["used_segment"] is not None and
                    "fps" in data_dict and data_dict["fps"] is not None):
                    begin_fid = data_dict["used_segment"]["begin_fid"]
                    end_fid = data_dict["used_segment"]["end_fid"]
                    fps = data_dict["fps"]
                    start_time = begin_fid / fps
                    end_time = end_fid / fps
                else:
                    start_time = None
                    end_time = None
                
                images, timestamps, imgs = load_video_new(
                    video_file,
                ...
                if ("ground_truth" in data_dict and data_dict["ground_truth"] is not None and
                    "bboxs" in data_dict["ground_truth"] and
                    "used_segment" in data_dict and data_dict["used_segment"] is not None and
                    "fps" in data_dict and data_dict["fps"] is not None):
        
                    begin_fid = data_dict["ground_truth"]["start_time"]
                    end_fid = data_dict["ground_truth"]["end_time"]
                    bboxs_list = data_dict["ground_truth"]["bboxs"]
                    num_bboxs = len(bboxs_list)
                    fps = data_dict["fps"]
                    bboxs = []
                    for i, ts in enumerate(timestamps):
                        frame_id = round(ts * fps)
                        if begin_fid <= frame_id <= end_fid:
                            index = min(max(frame_id - begin_fid, 0), num_bboxs - 1)
                            bbox_dict = bboxs_list[index]
                            if bbox_dict is not None:
                                bbox = [ts, bbox_dict['xmin'], bbox_dict['ymin'], bbox_dict['xmax'], bbox_dict['ymax']]
                                bboxs.append(bbox)
                                imgs_gt.append(imgs[i])
                                
                    if bboxs == []:
                        last_bbox_dict = bboxs_list[-1]
                        target_ts = end_fid / fps
                        rounded_ts = float(round(target_ts))
                        last_timestamp = min(timestamps, key=lambda x: abs(x - rounded_ts))
                        if last_bbox_dict is not None:
                            bbox = [last_timestamp, last_bbox_dict['xmin'], last_bbox_dict['ymin'], 
                                    last_bbox_dict['xmax'], last_bbox_dict['ymax']]
                            bboxs.append(bbox)
                            last_timestamp_index = timestamps.index(last_timestamp)
                            imgs_gt.append(imgs[last_timestamp_index])
                else:
                    bboxs = None
            else:
            ...
            if conv["from"] == "human":
                ...
                for chunk_idx in range(1, 2 * len(chunks)):
                    if chunk_idx % 2 == 1:
                        ...
                        if chunk:
                            messages[-1]["content"].append({"type": "text", "text": chunk})
                    else:
                        if modal == 'image':
        ...
        match = re.search(r'<query>(.*?)</query>', messages[0]['content'][1]['text'], re.DOTALL)
        caption = match.group(1)
        # Return original outputs plus bounding boxes
        return modal, images, messages, merge_size, bboxs, imgs_gt, caption
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            if "stream" in data_dict and data_dict["stream"]:
                ...
                modal, images, messages, merge_size, bboxs, imgs, caption = self._convert_normal(data_dict)
            ...
            target_sizes = data_dict['target_sizes']
            ori_sizes = data_dict['ori_sizes']
            data_dict['imgs'] = imgs
            data_dict['caption'] = caption
            if bboxs is not None:
                # Convert bboxs to tensor if not already
                bboxs = torch.as_tensor(bboxs, dtype=data_dict['pixel_values'].dtype)
                ...
                # Scale factors
                # scale_x = target_width / ori_width
                # scale_y = target_height / ori_height
                
                # Scale bounding boxes
                t = bboxs[:, 0]  # Keep t unchanged
                xmin = bboxs[:, 1]
                ymin = bboxs[:, 2]
                xmax = bboxs[:, 3]
                ymax = bboxs[:, 4]
                ...
                xmin = xmin / ori_width
                ymin = ymin / ori_height
                xmax = xmax / ori_width
                ymax = ymax / ori_height          
                
                # Combine into new bbox tensor
                new_bboxs = torch.stack([t, xmin, ymin, xmax, ymax], dim=1)
                
                # Save to data_dict
                data_dict['bboxs'] = new_bboxs
...
class DataCollatorWithFlatteningForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict], separator_id=-100) -> Dict[str, torch.Tensor]:
        ...
        batch["merge_sizes"] = torch.cat([x["merge_sizes"] for x in instances])
        batch["modals"] = sum([x["modals"] for x in instances], [])
        valid_bboxs = [x["bboxs"] for x in instances if len(x["bboxs"]) > 0]
        batch["bboxs"] = torch.cat(valid_bboxs) if valid_bboxs else []
        batch["imgs"] = [x["imgs"] for x in instances]
        batch["caption"] = [x["caption"] for x in instances]
...
def _find_latest_checkpoint(dir_path: str) -> Optional[str]:
    ckpts = list(pathlib.Path(dir_path).glob("checkpoint-*"))
    if not ckpts:
        return None
...
def _auto_merge_on_resume_if_any(model_args, training_args):
    latest_ckpt = _find_latest_checkpoint(training_args.output_dir)
    if not (latest_ckpt and training_args.auto_merge_on_resume):
        return latest_ckpt
...
def train(attn_implementation=None):
    ...
    latest_ckpt = _auto_merge_on_resume_if_any(model_args, training_args)
    ...
    config.g_dino_config_path = model_args.g_dino_config_path
    ...
    if model_args.vision_encoder is not None:
        ...
        # model.get_model().initialize_grounding_dino_modules()
        g_dino = model.get_model().get_g_dino()
        g_dino.to(device=training_args.device, dtype=compute_dtype)
        g_dino.train()
        criterion = model.get_model().get_g_dino_criterion()
        criterion.to(device=training_args.device, dtype=compute_dtype)
        criterion.train()
        ...
        model.config.g_dino_lr = training_args.g_dino_lr

        for param in model.get_model().g_dino.parameters():
        ...
        if model.config.g_dino_lr is None:
            for p in model.get_model().g_dino.parameters():
                p.requires_grad = False
        ...
        tokenizer.add_tokens([DEFAULT_IMAGE_TOKEN, DEFAULT_OUTPUT_BOX_TOKEN, DEFAULT_QUERY_START_TOKEN, DEFAULT_QUERY_END_TOKEN, STREAM_START_TOKEN, STREAM_END_TOKEN], special_tokens=True)
        model.config.image_token_index = tokenizer.convert_tokens_to_ids(DEFAULT_IMAGE_TOKEN)
        model.config.tbox_index = tokenizer.convert_tokens_to_ids(DEFAULT_OUTPUT_BOX_TOKEN)
        model.config.query_start_index = tokenizer.convert_tokens_to_ids(DEFAULT_QUERY_START_TOKEN)
        model.config.query_end_index = tokenizer.convert_tokens_to_ids(DEFAULT_QUERY_END_TOKEN)
        

        vlprocessor = DeViLProcessor(vision_encoder.image_processor, tokenizer)
    ...
    # select a Trainer
    trainer = DeViLTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
